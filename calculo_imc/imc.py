...
while continuar == 'S':
    ...
    print(f"{cor['roxo']}{'PROCESSANDO'}{cor['limpa']}", end='', flush=True)
    for c in range(1, 4):
        print(f"{cor['branco']}.{cor['limpa']}", end='', flush=True)
        sleep(1)
    ...
    continuar = str(input(f'{cor["amarelo"]}REALIZAR OUTRO CALCULO IMC?{cor["limpa"]} ' 
                      f'{cor["branco"]}{'[S/N]'}{cor["limpa"]} ')).upper()
    print(f"{cor['branco']}-{cor['limpa']}" * 25)